...
class Trainer(pl.LightningModule):
    def __init__(self, model, w_video, w_frame, dataset_type, batch_size, num_epochs, lr, **dataset_kwargs):
        ...
        self.video_conf_matrix = torchmetrics.ConfusionMatrix(
            num_classes=4, compute_on_step=False)
        self.frame_conf_matrix = torchmetrics.ConfusionMatrix(
            num_classes=4, compute_on_step=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_frame = F.cross_entropy(
            frame_logits.flatten(0, 1), y_frame.flatten())
        loss = self.w_video * loss_video + self.w_frame * loss_frame

        logs = {'loss': loss, 'loss_video': loss_video,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.frame_conf_matrix.update(frame_logits.flatten(
            0, 1).softmax(dim=1), y_frame.flatten())

        return {'val_loss': loss, 'val_loss_video': loss_video, 'val_loss_frame': loss_frame}

    def validation_epoch_end(self, outputs):
        avg_loss = torch.stack([x['val_loss'] for x in outputs]).mean()
        avg_loss_video = torch.stack(
            [x['val_loss_video'] for x in outputs]).mean()
        avg_loss_frame = torch.stack(
            [x['val_loss_frame'] for x in outputs]).mean()

        video_matrix = self.video_conf_matrix.compute()
        video_top1_acc = video_matrix.diagonal().sum() / (video_matrix.sum())
        video_mean_acc = (video_matrix.diagonal() /
                          (video_matrix.sum(dim=0))).mean()

        frame_matrix = self.frame_conf_matrix.compute()
        frame_top1_acc = frame_matrix.diagonal().sum() / (frame_matrix.sum())
        frame_mean_acc = (frame_matrix.diagonal() /
                          (frame_matrix.sum(dim=0))).mean()
